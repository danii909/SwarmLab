from __future__ import annotations
from pathlib import Path
import streamlit as st

def render_global_sidebar():
    with st.sidebar:
        st.header("⚙️ Configurazione globale")
        st.divider()

        st.subheader("📁 Istanza")
        instances_found = sorted(
            str(p) for p in Path(".").glob("**/*.json")
            if "Consegna" in str(p) or "instances" in str(p).lower()
        )
        if not instances_found:
            instances_found = ["Consegna/A.json", "Consegna/B.json", "A.json", "B.json"]
        
        instances_found = [p for p in instances_found if Path(p).exists()] or instances_found
        instance_path = st.selectbox("File istanza", options=instances_found, index=0)

        st.subheader("⚙️ Seed")
        seed = st.number_input("(−1 = casuale)", min_value=-1, max_value=9999, value=42)

        # --- CSS INJECTION: GLASSMORPHISM FOOTER ---
        st.markdown(
            """
            <style>
                /* Contenitore principale fissato in basso */
                .sidebar-footer-container {
                    position: fixed;
                    bottom: 25px;
                    left: 10px;
                    width: 280px;
                    padding: 15px;
                    background: rgba(255, 255, 255, 0.05);
                    backdrop-filter: blur(10px);
                    border-radius: 15px;
                    border: 1px solid rgba(255, 255, 255, 0.1);
                    box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1);
                    transition: all 0.3s ease-in-out;
                    z-index: 100;
                }

                .sidebar-footer-container:hover {
                    background: rgba(255, 255, 255, 0.1);
                    border: 1px solid rgba(255, 75, 75, 0.4); /* Glow rosso Streamlit */
                    transform: translateY(-5px);
                }

                .author-label {
                    font-size: 14px;
                    color: #9ca3af;
                    margin: 0;
                    font-weight: 500;
                    letter-spacing: 0.5px;
                }

                .author-name {
                    font-size: 22px;
                    font-weight: 800;
                    color: #ffffff;
                    margin: 5px 0 12px 0;
                    display: block;
                    background: white;
                    -webkit-background-clip: text;
                    -webkit-text-fill-color: transparent;
                }

                .github-btn {
                    display: flex;
                    align-items: center;
                    justify-content: center;
                    gap: 10px;
                    padding: 10px;
                    background-color: #1f2937;
                    color: #ffffff !important;
                    text-decoration: none;
                    border-radius: 8px;
                    font-size: 16px;
                    font-weight: 600;
                    transition: 0.2s;
                    border: 1px solid transparent;
                }

                .github-btn:hover {
                    background-color: #ff4b4b;
                    border-color: #ffffff;
                    transform: scale(1.02);
                    box-shadow: 0 0 15px rgba(255, 75, 75, 0.4);
                }
            </style>
            
            <div class="sidebar-footer-container">
                <p class="author-label">Created by</p>
                <span class="author-name">Daniele Barbagallo</span>
                <a class="github-btn" href="https://github.com/danii909" target="_blank">
                    <svg height="20" width="20" viewBox="0 0 16 16" fill="currentColor">
                        <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8"/>
                    </svg>
                    GitHub
                </a>
            </div>
            """,
            unsafe_allow_html=True
        )

    return instance_path, seed